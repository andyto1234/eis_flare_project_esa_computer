...
import pandas as pd
def get_closest_aia(date_time_obj, wavelength = 193, path = None, route = 'sunpy'):
    ...
    """
    if route == 'sunpy':
        aia_map_plot = _sunpy_search_and_fetch(date_time_obj, wavelength, path)
        return aia_map_plot
    elif route == 'aiapiper':
        _aiapiper_search_and_fetch(date_time_obj, wavelength, path)
        return None

def _sunpy_search_and_fetch(date_time_obj, wavelength, path = None):
    DATE_FORMAT = "%Y-%m-%d %H:%M:%S"
    ...
    path = path if path is not None else "./{instrument}/{file}"
    aia_downloads = Fido.fetch(aia_res, path=path)
    aia_map_plot = sunpy.map.Map(aia_downloads[0])
    return aia_map_plot

def _aiapiper_search_and_fetch(date_time_obj, wavelength, path = None):
    downloader = PipeFix()

    time_minus = (date_time_obj - pd.Timedelta(minutes=0.5)).strftime("%Y-%m-%dT%H:%M:%S.000")
    time_plus = (date_time_obj + pd.Timedelta(minutes=0.5)).strftime("%Y-%m-%dT%H:%M:%S.000")

    downloader.fetch(
        start_date=time_minus,
        end_date=time_plus,
        wavelength=wavelength,
        cadence=1*u.min, # Can also use u.hour or u.min
